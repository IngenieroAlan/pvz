class GameController {
    ...
    if (
      ...
      if (col && row && !alredyPlanted) {
        switch (this.selectedPlant) {
          case "sunflower":
            plants.push(
              new Sunflower(sunflower, col, row, sunSprite, getSunSound)
            );
          ...
          case "potatomine":
            plants.push(
              new PotatoMine(potatomine, col, row, potatoExplotionSound)
            );
  ...
  applyBuffs() {
    for (let plant of plants) {
      for (let buff of this.buffs) {
        buff.applyBuff(plant);
      }
    }
  }

  collectBuff(mouseX, mouseY) {
    for (let buff of this.buffs) {
      if (buff.isCollected(mouseX, mouseY)) {
        this.applyBuffs();
        return;
      }
    }
